...
class UserController extends Controller
{
    ...
    public function viewUser(Request $request, $id)
    {
        ...
        if ($user_details == null) {
            ...
            // Queries
            $contents = Generate::where('generate_by', $user_details->id)->orderBy('id', 'desc')->paginate(10);
            $images = AiImages::where('generate_by', $user_details->id)->orderBy('id', 'desc')->paginate(10);
            $settings = Setting::where('status', 1)->first();
    ...
    public function UpdateUserPlan(Request $request)
    {
        ...
        // Paid amount
        $amountToBePaid = ((int)($plan_data->price) * (int)($config[25]->config_value) / 100) + (int)($plan_data->price);
        ...
        if ($user_details->plan_validity == "") {
            ...
            $invoice_details['subtotal'] = $plan_data->price;
            $invoice_details['tax_amount'] = (int)($plan_data->price) * (int)($config[25]->config_value) / 100;
            ...
            $details = [
                ...
                'tax_amount' => '0',
                'invoice_amount' => $plan_data->price,
                'invoice_id' => $config[15]->config_value . $invoice_number,
                'invoice_date' => Carbon::now(),
                'description' => $plan_data->name . ' plan Upgrade',
            ...
            // Page redirect
            return redirect()->route('admin.pages.offline.transactions')->with('success', trans('Plan changed successfully!'));
        } else {
            $message = "";
