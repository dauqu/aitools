        ...
        <div class="page-header d-print-none">
            <div class="row align-items-center">
                <div class="col">
                    ...
                    <h2 class="page-title">
                        {{ __('Recently Generated Speech to Text') }}
                    </h2>
                </div>
            </div>
        </div>
            ...
            <div class="row row-cards row-deck mb-3">
                ...
                <div class="col-md-6 col-lg-3">
                    <div class="card border border-muted rounded">
                        <a href="{{ route('user.new.ai.speech.to.text') }}" class="text-muted">
                            <div class="card-body text-center p-5" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                title="{{ __('New Speech to text') }}">
                ...
                {{-- Generated Speech to text --}}
                @foreach ($speechToTexts as $speechToText)
                <div class="col-md-6 col-lg-3">
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex align-items-center mb-3">
                                <span class="badge bg-red text-capitalize">{{ str_replace('-', ' ', $speechToText->type)
                                    }}</span>
                                {{-- Actions --}}
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-end">
                                            <a class="dropdown-item" data-bs-toggle="tooltip" data-bs-placement="right"
                                                title="{{ __('View') }}"
                                                href="{{ route('user.view.ai.speech.to.text', $speechToText->generate_id) }}">{{
                                                __('View')}}</a>
                                            <a class="dropdown-item" data-bs-toggle="tooltip" data-bs-placement="right"
                                                title="{{ __('Edit') }}"
                                                href="{{ route('user.edit.ai.speech.to.text', $speechToText->generate_id) }}">{{
                                                __('Edit')}}</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <h3 class="card-title text-uppercase my-1">{{ mb_strimwidth($speechToText->name, 0, 32,
                                "...") }}
                            </h3>
                            <div class="text-muted">
                                {{ __('Allocated Words') }} : {{ $speechToText->word_count }}
                            </div>
                        </div>
                        <div class="card-footer">
                            <div class="d-flex">
                                ...
                                <strong>{{ __('Last Updated on') }}: <span class="text-primary">{{
                                        $speechToText->updated_at->diffForHumans()
                                        }}</span></strong>
                            </div>
                        </div>
                    </div>
